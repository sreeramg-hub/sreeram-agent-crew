...
def main() -> None:
    ...
    for metal, ticker in METALS.items():
        try:
            ...
            change = current - last
            print(f"{metal}: current=${current:.2f}, last seen=${last:.2f}, change=${change:+.2f}")
    ...
    # Always update last-seen with current prices so next run compares fresh
    merged = {**last_seen, **current_prices}
    save_last_seen(merged)
